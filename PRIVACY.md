...
Tab Eagle stores your preferred sort mode locally in Chrome extension storage. If you rename a window in Tab Eagle, that label is stored in extension session storage and clears when Chrome restarts.
...
Tab Eagle stores your preferred sort mode using Chrome's local extension storage. User-created window labels use session storage and clear when Chrome restarts. You can remove stored extension data by uninstalling the extension or clearing extension site data through Chrome.
...
- `storage`: required to save your preferred sort mode locally and window labels for the current Chrome session.
